...
| `is_clean_build` | Runs `clean` before `test`, so the test build starts from nothing. It makes the Step slower, and is only needed when a stale build product would otherwise be reused. | required | `yes` |
| `generate_code_coverage_files` | Tells xcodebuild to produce coverage data by setting `GCC_GENERATE_TEST_COVERAGE_FILES=YES` and `GCC_INSTRUMENT_PROGRAM_FLOW_ARCS=YES`. | required | `no` |
...
| --- | --- |
| `BITRISE_XCODE_TEST_RESULT` | Whether the test run passed: `succeeded` or `failed`. It is exported even when the Step fails, so a later Step can branch on it. |
</details>
